...
export async function maintainOemMapping(
    ...
    for (const p of data) {
        if (p?.oem_product_id && p?.product_id) {
            byOem.set(p.oem_product_id, p.product_id);
        }
    }

    const pairs = Array.from(byOem.entries()).map(([oem_product_id, product_id]) => ({ oem_product_id, product_id }));
    if (pairs.length === 0) return { totUpserted: true, missing: [] };
    ...
    try {
        ...
        for (const c of chunk(pairs, UPSERT_ROWS_PER_CHUNK)) {
            ...
            const sql = `INSERT INTO oem_product_map (oem_product_id, product_id)
    VALUES ${placeholders}
    ...
    `;
            ...
            totUpserted += c.length;
        }

        await db.execute('COMMIT;');

        const current = await db.select<OemMapping[]>(
            'SELECT oem_product_id, product_id FROM oem_product_map'
        );

        const incomingOems = new Set(pairs.map(p => p.oem_product_id));
        const missing = current.filter(row => !incomingOems.has(row.oem_product_id)); // keep full rows for return

        return { totUpserted, missing };
    } catch (e: any) {
...
/**
 * Batch upsert product_defect_map rows.
 * - Chunks to stay under SQLite parameter limits
...
export async function upsertManyProductDefectMaps(
    ...
    const ROWS_PER_CHUNK = Math.max(1, Math.floor(MAX_PARAMS / PARAMS_PER_ROW));

    const chunks: ProductDefectMapRow[][] = [];
    ...
    try {
        for (const c of chunks) {
            const placeholders = c.map(() => '(?, ?, ?, ?, ?)').join(', ');
            const sql = `INSERT INTO product_defect_map
    (product_id, lot_id, wafer_id, sub_id, file_path)
    VALUES ${placeholders}
    ON CONFLICT(product_id, lot_id, wafer_id) DO UPDATE SET
        sub_id = excluded.sub_id,
